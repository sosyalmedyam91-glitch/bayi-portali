...
export default function DocumentCenterPage() {
  ...
  const last30Days = documents.filter((doc) => {
    if (!doc.updatedAt) return false;

    const fileDate = new Date(doc.updatedAt);
  ...
  const financeDocuments = documents.filter(
    (doc) => doc.category === "Finans & Hukuk",
  ).length;

  const hrDocuments = documents.filter(
    (doc) => doc.category === "İK & Yönetmelik",
  ).length;
  ...
  async function handleShare(doc: any) {
    if (!doc.url) {
      alert("Paylaşılacak bağlantı bulunamadı.");
      return;
    }
  ...
  async function handleDelete(doc: any) {
    const ok = confirm(`${doc.title} silinsin mi?`);
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-[#53575A]">
            Kurumsal dökümanları, kılavuzları ve şablonları güvenli bir şekilde
            depolayın ve paylaşın.
          </p>
        </div>
        <div className="flex gap-2 w-full sm:w-auto">
          ...
          >
            ...
            <option value="finans-hukuk">Finans & Hukuk</option>

            <option value="ik">İK & Yönetmelik</option>

            <option value="teknik">Üretim / Teknik</option>
          </select>

          <button
            onClick={() => fileInputRef.current?.click()}
            disabled={uploading || !category}
            className={`group flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 shadow-sm
${
  uploading || !category
    ? "bg-gray-300 text-white cursor-not-allowed"
    : "bg-[#EA0029] hover:bg-[#c40022] hover:shadow-lg hover:scale-105 cursor-pointer text-white"
}`}
          >
            {/* Yükle (Upload) İkonu */}
                        ...
                        <div
                          className={`p-2 rounded-lg ${
                            doc.type === "PDF"
                              ...
                              : doc.type === "DOCX"
                                ? "bg-[rgba(234,0,41,0.06)] text-[#EA0029]"
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <div className="flex justify-end gap-1">
                        ...
                        <button
                          ...
                          className="p-1.5 text-[#53575A] hover:text-gray-900 hover:bg-gray-100 rounded-md transition-colors"
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="16"
