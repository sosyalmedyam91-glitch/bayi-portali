...
const dummyNotifications = [
  ...
  { id: "NTF-004", title: "Sistem Güncellemesi", message: "ERP v2.4.0 sürümü başarıyla canlıya alındı. Yeni raporlama modülleri aktif.", type: "Info", time: "Dün", read: true },
];
    ...
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-[#53575A]">Sistem uyarıları, finansal hareketler ve operasyonel güncellemelerden anında haberdar olun.</p>
        </div>
        <button 
          onClick={markAllAsRead}
          className="inline-flex items-center gap-2 text-sm font-semibold text-[#EA0029] hover:text-[#c40022] bg-[rgba(234,0,41,0.06)] hover:bg-[rgba(234,0,41,0.1)] px-4 py-2 rounded-lg transition-colors"
        >
          {/* Çift Tik İkonu */}
      ...
      <div className="flex p-1 bg-gray-100 rounded-xl w-fit text-sm font-medium text-[#53575A]">
        <button 
      ...
      <div className="space-y-3">
        {filteredNotifications.length === 0 ? (
          <div className="text-center p-12 bg-white border rounded-xl shadow-sm text-gray-400 space-y-2">
            ...
            <p className="font-medium text-sm text-[#53575A]">Görüntülenecek bildirim bulunmuyor.</p>
          </div>
        ) : (
          filteredNotifications.map((ntf) => (
            <div 
              ...
              className={`p-4 border rounded-xl shadow-sm flex items-start gap-4 transition-all bg-white hover:border-gray-300 ${
                !ntf.read ? "ring-1 ring-[rgba(234,0,41,0.15)] border-[rgba(234,0,41,0.25)] bg-[rgba(234,0,41,0.01)]" : ""
              }`}
            >
              {/* Dinamik Renkli Sol İkon Alanı (Durum Renkleri Korundu, Info Kurumsal Kırmızı Yapıldı) */}
              <div className={`p-2.5 rounded-xl shrink-0 ${
                ntf.type === "Error" ? "bg-red-50 text-red-600" :
                ntf.type === "Success" ? "bg-emerald-50 text-emerald-600" :
                ntf.type === "Warning" ? "bg-amber-50 text-amber-600" : "bg-[rgba(234,0,41,0.06)] text-[#EA0029]"
              }`}>
              ...
              <div className="flex-1 space-y-1">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-1">
                  <div className="flex items-center gap-2">
                    ...
                    {!ntf.read && (
                      <span className="h-2 w-2 bg-[#EA0029] rounded-full" title="Okunmadı"></span>
                    )}
                  </div>
                  <span className="text-xs text-gray-400 font-medium">{ntf.time}</span>
                </div>
                <p className="text-sm text-[#53575A] leading-relaxed">{ntf.message}</p>
              </div>
              ...
              {!ntf.read && (
                <button 
                  ...
                  className="p-1.5 text-gray-400 hover:text-[#EA0029] hover:bg-[rgba(234,0,41,0.06)] rounded-lg transition-colors shrink-0"
                >
